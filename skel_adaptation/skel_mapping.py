...
def main_forward(args):
    ...
    for ann in annotations:
        ...
        split_out_dir = os.path.join(args.out_dir, 'forw', split_name)
        os.makedirs(split_out_dir, exist_ok=True)
        out_path = os.path.join(split_out_dir, f"{frame_dir}.npy")
        np.save(out_path, smpl_joints)
        saved_count += 1

    print(f"✅ Saved {saved_count} training sequences to {args.out_dir} (organized by split)")

def main_backward(args):
    
    split_dirs = [d for d in os.listdir(args.input_data) if os.path.isdir(os.path.join(args.input_data, d))]
    annotations = []
    split = {}

    for split_name in split_dirs:
        split_path = os.path.join(args.input_data, split_name)
        npy_files = [f for f in os.listdir(split_path) if f.endswith('.npy')]
        split_key = split_name + '_train'
        split[split_key] = []

        for npy_file in npy_files:
            sample_name = npy_file.replace('.npy', '')
            npy_path = os.path.join(split_path, npy_file)
            ...
            label = int(sample_name[-4:].replace('A', '')) - 1  # Extract label from 'A###'
            annotations.append({
            ...
            split[split_key].append(sample_name)
    ...
    print(f"✅ Reconstructed NTU annotations saved to {out_pkl_path}")
